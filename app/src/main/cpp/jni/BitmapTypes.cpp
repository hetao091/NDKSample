...
//    memset(&bitmapInfo , 0 , sizeof(bitmapInfo));
    ...
    LOGD("handleBitmapForSinglePixel bitmap width = %d", mWidth)
    LOGD("handleBitmapForSinglePixel bitmap height = %d", mHeight)
    LOGD("handleBitmapForSinglePixel bitmap format = %d", bitmapInfo.format)
    int a, r, g, b;
    for (int x = 0; x < mWidth; ++x) {

        for (int y = 0; y < mHeight; ++y) {
            LOGD("handleBitmapForSinglePixel %d", pixelArr[0])
            void *pixel = nullptr;
            // 移动像素指针
            pixel = pixelArr + y * mWidth + x;
            //按照ABGR存储序列取值  获取指针对应的值
...
extern "C"
JNIEXPORT void JNICALL
Java_tt_reducto_ndksample_jni_BitmapOps_rotateBitmapWithPointer(JNIEnv *env, jobject thiz,
                                                                jobject bitmap, jint ops) {
    ...
    // 矩阵 创建一个新的数组指针填充像素值
    auto *newBitmapPixels = new uint32_t[mWidth * mHeight];
    LOGD("rotateBitmapWithPointer bitmap width = %d", mWidth)
    LOGD("rotateBitmapWithPointer bitmap height = %d", mHeight)
    LOGD("rotateBitmapWithPointer bitmap format = %d", bitmapInfo.format)
    int temp = 0;
    switch (ops) {
        case 0:
            LOGD("rotateBitmapWithPointer rotate 90")
            // 遍历矩阵，按照顺时针90度顺序扫描
            for (int x = 0; x < mWidth; x++) {
                for (int y = mHeight - 1; y >= 0; --y) {
...
//            }
    ...
    int size = mWidth * mHeight;
    // 写入新值
    memcpy((uint32_t *) pixelArr, newBitmapPixels, sizeof(uint32_t) * size);
    delete[] newBitmapPixels;
    AndroidBitmap_unlockPixels(env, bitmap);
}
